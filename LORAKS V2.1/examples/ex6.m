%% example 5: Comparison of different amounts of iteration
clear;
close all;
clc
...
for max_iter = [5:5:65]
    ...
    tic
    recon = AC_LORAKS(undersampledData, kMask, rank, [], [], [],[],[],max_iter);
    time = toc;
    ...
    caxis([0,1.3]);
    title(['max\_iter = ' num2str(max_iter) ', NRMSE = ' num2str(norm(rGold(:)-rSoS(:))/norm(rGold(:))) ', time = ' num2str(time) ' seconds']);
    disp(' ');
end